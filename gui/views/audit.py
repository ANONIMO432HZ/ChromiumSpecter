...
class AuditView(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # Timeout Webhooks
        make_label(r1, "Timeout Webhooks (s):", style="small", color=COLORS["text_secondary"]).pack(side="left", padx=(0, PAD["sm"]))
        self._timeout_var = ctk.StringVar(value="15")
        make_entry(r1, placeholder="15", width=50, textvariable=self._timeout_var).pack(side="left")
        ...
        make_button(r3, "🗑  Limpiar Consola",  command=self._clear_log,   style="secondary", width=140).pack(side="left", padx=(0, PAD["sm"]))
        ...
        self._log = ctk.CTkTextbox(
            ...
            border_color=COLORS["border"], wrap="word", state="disabled",
        )
    ...
    def _clear_log(self):
        ...
        self._log.configure(state="disabled")
        try: self.winfo_toplevel().clipboard_clear()
        except: pass
    ...
    def _audit_worker(self):
        ...
        try: timeout = int(self._timeout_var.get() or 15)
        except: timeout = 15
        ...
        try:
            ...
            results, hp, cp = decryptor.audit(
                ...
                webhook_timeout=timeout # AHORA SE USA PARA WEBHOOKS
            )
            ...
            if results:
                ...
                if self._on_results_cb: self._after_call(lambda: self._on_results_cb(results, hp, cp))
            else:
                self._stats["total"] = 0

            self._after_call(lambda: self._set_duration(elapsed))
            self._after_call(self._refresh_stats)
            self._log_line("ACCENT", f"Proceso finalizado en {elapsed:.2f}s")

        except Exception as exc:
            self._log_line("ERR", f"Falla crítica: {exc}")
        finally:
